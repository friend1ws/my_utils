...
# query: query sequences (fasta format)
# target: target sequences (fasta format)
# min_score: minimum score for the query to be counted
def main2(query, target, min_score, nMatch = 2, nMismatch = 2, nOpen = 3, nExt = 1):
    ...
    ##########    
    # get template seq names
    template_keys = []
    with open(target, 'r') as hin:
        for line in hin:
            if line.startswith(">"):
                template_keys.append(line.lstrip('>').rstrip('\n'))
    ##########

    type2count_zero = {}
    for key in template_keys:
        type2count_zero[key] = 0

    # initialize the template counts
    type2count = copy.deepcopy(type2count_zero)
    ...
    for sQId,sQSeq,sQQual in read(query):
        ...
        # iter target sequence
        type2max_score = copy.deepcopy(type2count_zero)
